package net.ruffy.marble.graphics3d {
  ...
  object Camera {
    ...
    /**
      ...
      */
    def fromDirection( direction: Vector3, rotation: Float ): (Vector3, Vector3, Vector3) = {
    ...
    def fromPolar( r: Float, theta: Float, phi: Float ): Vector3 = Vector3( r*sin( theta )*cos( phi ), r*cos( theta ), r*sin( theta )*sin( phi ) )
