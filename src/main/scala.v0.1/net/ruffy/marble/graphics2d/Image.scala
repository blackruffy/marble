package net.ruffy.marble.graphics2d {
  ...
  object Image {
    ...
    class ColorType( underlying: ByteBuffer ) extends Type[Color]( underlying ) {
      ...
      def put( c: Color ) = {
        underlying.putFloat( c.alpha )
        underlying.putFloat( c.red )
        underlying.putFloat( c.green )
        underlying.putFloat( c.blue )
      }
      def color( c: Color ) = c
  ...
  class ImageBuffer[A] protected ( val width: Int, val height: Int, buf: ByteBuffer, typeSize: Int, wrap: ByteBuffer => Image.Type[A] ) extends Image[A] {
    ...
    private def position( x: Int, y: Int ): Unit = getPosition(x, y) |> buf.position

    private def get:A  = wrapper.get
