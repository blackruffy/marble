package net.ruffy.marble.graphics3d.modeler {
  ...
  trait PolygonEnv {
    ...
    abstract class Polygon extends Shape3D {
      ...
      protected def create( points: List[Vector3], normals: List[Vector3], material: Material ): A
      ...
      def drawWireFrame( g: Graphics, camera: Camera, color: Int ): Unit = {
        ...
        val pixs: List[(Int, Int, Int)] = points.map( camera.toScreen _ >> screen.pixel _ )
