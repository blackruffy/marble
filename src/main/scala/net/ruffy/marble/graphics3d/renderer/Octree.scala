package net.ruffy.marble.graphics3d.renderer {
  ...
  import net.ruffy.marble.graphics2d.{ ImageEnv, ColorEnv }
  import net.ruffy.marble.graphics3d.modeler.{ Shape3DEnv, BoundingSphereEnv }
  ...
  trait OctreeEnv {
    ...
    abstract class Octree[B, C <: Octree[B, C]](
      val xmin: Num, val xmax: Num,
      val ymin: Num, val ymax: Num,
      val zmin: Num, val zmax: Num,
      val capacity: Int,
      val parent: Option[C]
    ) {
      ...
      lazy val xcenter: Num = (xmin + xmax)/2
      lazy val ycenter: Num = (ymin + ymax)/2
      lazy val zcenter: Num = (zmin + zmax)/2
      ...
      def withinx( x: Num ): Boolean = x >= xmin && x < xmax
      def withiny( y: Num ): Boolean = y >= ymin && y < ymax
      def withinz( z: Num ): Boolean = z >= zmin && z < zmax
    ...
    object Octree {
      ...
      def apply[B, C <: Octree[B, C]](
        ...
        create: (Num, Num, Num, Num, Num, Num, Int) => C
